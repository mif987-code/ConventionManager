    ...
    try {
        ...
        if (response.data && response.data.data) {
            const cards = response.data.data.map((card) => ({
                ...
                collector_number: card.collector_number,
                foil: card.foil,
            })).sort((a, b) => a.collector_number.localeCompare(b.collector_number, undefined, { numeric: true }));
            res.json({ cards });
        }
        else {
            res.json({ cards: [] });
...
const printsSearchUriCache = new Map();
// GET /api/cards/sets-by-card - Get all sets that contain a specific card
router.get('/sets-by-card', async (req, res) => {
    try {
        ...
        if (printsSearchUriCache.has(name)) {
            ...
            try {
                const namedRes = await axios_1.default.get(`https://api.scryfall.com/cards/named?exact=${encodeURIComponent(name)}`, {
                    headers: {
        ...
        if (printsSearchUri) {
            try {
                ...
                while (next) {
                    const response = await axios_1.default.get(next, {
                        headers: {
                            'User-Agent': 'ConventionManager/1.0',
                            'Accept': 'application/json',
                        },
                    ...
                    if (data.data) {
                        for (const card of data.data) {
                            ...
                            if (EXCLUDE_LAYOUTS.has(card.layout))
                                continue;
                            if (EXCLUDE_SET_TYPES.has(card.set_type))
                                continue;
        ...
        const result = Array.from(setsMap.values()).sort((a, b) => (b.released_at || '').localeCompare(a.released_at || ''));
