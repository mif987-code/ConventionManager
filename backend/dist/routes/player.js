...
router.post('/auth/login', async (req, res, next) => {
    try {
        ...
        if (!user.password_hash)
            return res.status(400).json({ error: 'Password not set. Please ask an organizer or use NFC login.' });
...
router.get('/me', playerAuth, async (req, res, next) => {
    try {
        ...
        if (user.convention_id) {
            const convRes = await db_1.pool.query(`SELECT id, name FROM conventions WHERE id = $1`, [user.convention_id]);
