...
router.get('/auth/config', async (_req, res, next) => {
    try {
        let result = await db_1.pool.query(`SELECT scan_mode FROM conventions WHERE status = 'active' ORDER BY created_at DESC LIMIT 1`);
        if (result.rows.length === 0)
            result = await db_1.pool.query(`SELECT scan_mode FROM conventions ORDER BY created_at DESC LIMIT 1`);
...
router.post('/auth/forgot-password', passwordResetLimiter, async (req, res, next) => {
    try {
        ...
        const result = await db_1.pool.query(`SELECT id, name, last_name, email FROM users WHERE lower(email) = $1 LIMIT 1`, [email]);
        ...
        if (user) {
            ...
            await db_1.pool.query(`INSERT INTO password_reset_tokens (user_id, token_hash, expires_at) VALUES ($1, $2, NOW() + INTERVAL '1 hour')`, [user.id, tokenHash]);
            ...
            await (0, emailService_1.sendPasswordResetEmail)(user.email, `${user.name}${user.last_name ? ` ${user.last_name}` : ''}`, resetUrl);
        }
...
router.post('/auth/reset-password', passwordResetLimiter, async (req, res, next) => {
    ...
    try {
        ...
        if (!resetToken || password.length < 8)
            return res.status(400).json({ error: 'El enlace y una contraseña de al menos 8 caracteres son obligatorios' });
        const tokenHash = (0, crypto_1.createHash)('sha256').update(resetToken).digest('hex');
        await client.query('BEGIN');
        const result = await client.query(`SELECT id, user_id FROM password_reset_tokens WHERE token_hash = $1 AND used_at IS NULL AND expires_at > NOW() FOR UPDATE`, [tokenHash]);
        if (result.rows.length === 0) {
...
router.post('/auth/nfc', authLimiter, async (req, res, next) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ error: 'No player found with that NFC tag' });
...
router.post('/auth/login', authLimiter, async (req, res, next) => {
    try {
        ...
        const user = result.rows[0];
        if (!user)
            return res.status(404).json({ error: 'No existe un jugador con ese correo' });
...
router.get('/me', playerAuth, async (req, res, next) => {
    try {
        ...
        if (user.convention_id) {
            const convRes = await db_1.pool.query(`SELECT id, name, scan_mode FROM conventions WHERE id = $1`, [user.convention_id]);
       ...
       ORDER BY sva.awarded_at DESC`, [userId]);
        ...
        const merchandiseRes = await db_1.pool.query(`SELECT id, item_name, image_url, is_claimed, claimed_at
...
router.put('/me/password', playerAuth, async (req, res, next) => {
    try {
        const userId = req.playerId;
        ...
        const hash = await bcrypt_1.default.hash(password, 10);
        await db_1.pool.query(`UPDATE users SET password_hash = $1, updated_at = NOW() WHERE id = $2`, [hash, userId]);
...
router.post('/regenerate-qr', playerAuth, async (req, res, next) => {
    try {
        const userId = req.playerId;
...
router.get('/events', playerAuth, async (req, res, next) => {
    try {
        const userId = req.playerId;
        const result = await db_1.pool.query(`SELECT ep.*, e.name AS event_name, e.status, e.created_at AS event_date,
...
router.get('/events/:id', playerAuth, async (req, res, next) => {
    try {
        const userId = req.playerId;
...
router.post('/events/:id/register', playerAuth, async (req, res, next) => {
    try {
        const userId = req.playerId;
        const eventId = parseInt(req.params.id);
...
router.post('/store/purchase', playerAuth, async (req, res, next) => {
    try {
        const userId = req.playerId;
...
router.get('/store/orders', playerAuth, async (req, res, next) => {
    try {
        const userId = req.playerId;
