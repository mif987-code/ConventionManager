...
router.post('/:id/send-password-reset', async (req, res, next) => {
    try {
        ...
        if (isNaN(userId))
            return res.status(400).json({ error: 'Invalid user ID' });
        const result = await db_1.pool.query(`SELECT id, name, last_name, email FROM users WHERE id = $1`, [userId]);
        ...
        await db_1.pool.query(`UPDATE password_reset_tokens SET used_at = NOW() WHERE user_id = $1 AND used_at IS NULL`, [userId]);
