...
exports.sendQRCodeEmail = sendQRCodeEmail;
 ...
 */
async function sendQRCodeEmail(to, userName, qrCodeDataUrl) {
    if (!resend) {
 ...
 */
async function sendPasswordResetEmail(to, userName, resetUrl, throwOnError = false) {
    if (!to)
        return false;
    if (!resend) {
        if (throwOnError)
            throw new Error('RESEND_API_KEY is not configured on the backend service.');
        return false;
    }
    try {
        const result = await resend.emails.send({
...
async function sendActivationEmail(to, userName) {
    if (!resend) {
        console.warn('[EmailService] RESEND_API_KEY is not configured. Skipping email.');
        return false;
    }
