...
router.post('/register', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (package_id) await packageService.validatePackageMerchandiseStock(package_id, 1);
...
router.post('/:id/send-password-reset', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const delivered = await sendPasswordResetEmail(user.email, `${user.name}${user.last_name ? ` ${user.last_name}` : ''}`, resetUrl, true);
    if (!delivered) {
