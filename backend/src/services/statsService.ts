...
export async function getStats(conventionId: number) {
  ...
  const totalPlayers = await pool.query(
    `SELECT COUNT(*)::int AS count FROM users WHERE convention_id = $1`,
    [conventionId]
  );

  const playerNames = await pool.query(
    `SELECT id, name, email FROM users WHERE convention_id = $1 ORDER BY name`,
    [conventionId]
  );

  // 2. Total number of events that were run
  const totalEvents = await pool.query(
    `SELECT COUNT(*)::int AS count FROM events WHERE convention_id = $1`,
    [conventionId]
  );

  // 3a. Breakdown of what types of events
  const eventTypeBreakdown = await pool.query(
    `SELECT et.category, et.name as type_name, COUNT(e.id)::int as event_count
     FROM event_types et
  ...
  const playersPerEvent = await pool.query(
    `SELECT e.id, e.name, COUNT(ep.user_id)::int as player_count
     FROM events e
  ...
  const tixAwarded = await pool.query(
    `SELECT COALESCE(SUM(amount), 0)::int AS total
     FROM transactions 
     WHERE type = 'tix' AND amount > 0 AND convention_id = $1`,
    [conventionId]
  );

  const tixUsed = await pool.query(
  ...
  const tixUsageByProduct = await pool.query(
    `SELECT si.name as product_name, COUNT(so.id)::int as order_count, 
     ...
     GROUP BY si.name
     ORDER BY total_tix_spent DESC`,
    [conventionId]
  );

  // 5. Products sold in real currency
  const realCurrencySales = await pool.query(
    `SELECT si.name as product_name, COUNT(so.id)::int as order_count,
  ...
  const tixPurchases = await pool.query(
    `SELECT COUNT(*)::int AS count, COALESCE(SUM(total_tix), 0)::int AS total_tix
     FROM store_orders 
     WHERE order_type = 'purchase' AND status != 'cancelled' AND convention_id = $1`,
    [conventionId]
  );

  const currencyPurchases = await pool.query(
    `SELECT COUNT(*)::int AS count, COALESCE(SUM(si.cost), 0)::numeric AS total_currency
     FROM store_orders so
     JOIN store_items si ON so.item_id = si.id
     WHERE so.order_type = 'purchase' AND so.status != 'cancelled'
       AND si.cost > 0 AND so.convention_id = $1`,
  ...
  const eventsOngoing = await pool.query(
    `SELECT COUNT(*)::int AS count FROM events WHERE status = 'ongoing' AND convention_id = $1`,
    [conventionId]
  );

  const eventsOpen = await pool.query(
    `SELECT COUNT(*)::int AS count FROM events WHERE status = 'open' AND convention_id = $1`,
    [conventionId]
  );

  const totalRegistrations = await pool.query(
    `SELECT COUNT(*)::int AS count FROM event_participants WHERE convention_id = $1`,
    [conventionId]
  );

  const storeReservations = await pool.query(
    `SELECT COUNT(*)::int AS count
     FROM store_orders WHERE order_type = 'reserve' AND status = 'reserved' AND convention_id = $1`,
    [conventionId]
  );
