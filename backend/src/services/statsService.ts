...
export async function getStats() {
  const eventsPlayed = await pool.query(
    `SELECT COUNT(*)::int AS count FROM events WHERE status = 'finished'`
  );

  const eventsOngoing = await pool.query(
    `SELECT COUNT(*)::int AS count FROM events WHERE status = 'ongoing'`
  );

  const eventsOpen = await pool.query(
    `SELECT COUNT(*)::int AS count FROM events WHERE status = 'open'`
  );
  ...
  const storePurchases = await pool.query(
